...
goal(deliver_box(box1,area21)).
goal(deliver_box(box2,area27)).
goal(deliver_box(box3,area39)).
goal(deliver_box(box4,area24)).
goal(deliver_box(box5,area20)).
goal(deliver_box(box6,area6)).
goal(deliver_box(box7,area35)).
goal(deliver_sample(cdm2,area7)).
goal(deliver_sample(cdm1,area26)).
goal(deliver_sample(cdm3,area40)).
goal(deliver_sample(cdm3,area28)).
goal(deliver_sample(cdm4,area2)).
goal(deliver_sample(cdm1,area32)).
goal(deliver_sample(cdm4,area22)).
...
+!replan(Goal,Role) : count(C) & replan(R) & C + R == 23 & round(Round) & time(Start)
<-
	-count(C);
	+count(0);
	if (R == 0) {
...
+!count : count(C) & C == 23 & time(Start) & replan(0) & round(Round)
<-
	jia.time(Start,Final);
	+time_ind_plan((Final-Start)/1000000000.0);
	.print("Elapsed time individual planning round ",Round,": ",(Final-Start)/1000000000.0);
	...
	.stopMAS;
	.
+!count : count(C) & replan(R) & C + R == 23  & round(Round) & time(Start)
<-
	-count(C);
	+count(0);
	if (R == 0) {
