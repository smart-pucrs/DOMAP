...
goal(deliver_box(box2,area20)).
goal(deliver_box(box3,area11)).
goal(deliver_sample(cdm2,area13)).
goal(deliver_sample(cdm1,area16)).
goal(deliver_sample(cdm2,area5)).
...
+!replan(Goal,Role) : count(C) & replan(R) & C + R == 11 & round(Round) & time(Start)
<-
	-count(C);
	+count(0);
	if (R == 0) {
...
+!count : count(C) & C == 11 & time(Start) & replan(0) & round(Round)
<-
	jia.time(Start,Final);
	+time_ind_plan((Final-Start)/1000000000.0);
	.print("Elapsed time individual planning round ",Round,": ",(Final-Start)/1000000000.0);
	...
	.stopMAS;
	.
+!count : count(C) & replan(R) & C + R == 11  & round(Round) & time(Start)
<-
	-count(C);
	+count(0);
	if (R == 0) {
