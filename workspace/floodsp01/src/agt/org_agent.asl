...
goal(deliver_box(box1,area9)).
goal(deliver_box(box2,area11)).
goal(deliver_sample(cdm2,area8)).
...
+!replan(Goal,Role) : count(C) & replan(R) & C + R == 8 & round(Round) & time(Start)
<-
	-count(C);
	+count(0);
	if (R == 0) {
...
+!count : count(C) & C == 8 & time(Start) & replan(0) & round(Round)
<-
	jia.time(Start,Final);
	+time_ind_plan((Final-Start)/1000000000.0);
	.print("Elapsed time individual planning round ",Round,": ",(Final-Start)/1000000000.0);
