...
goal(deliver_box(box1,area10)).
goal(deliver_box(box2,area35)).
goal(deliver_box(box3,area6)).
goal(deliver_box(box4,area31)).
goal(deliver_box(box5,area8)).
goal(deliver_box(box6,area7)).
goal(deliver_sample(cdm2,area10)).
goal(deliver_sample(cdm1,area22)).
goal(deliver_sample(cdm3,area32)).
goal(deliver_sample(cdm3,area17)).
goal(deliver_sample(cdm4,area25)).
goal(deliver_sample(cdm1,area9)).
...
+!replan(Goal,Role) : count(C) & replan(R) & C + R == 20 & round(Round) & time(Start)
<-
	-count(C);
	+count(0);
	if (R == 0) {
...
+!count : count(C) & C == 20 & time(Start) & replan(0) & round(Round)
<-
	jia.time(Start,Final);
	+time_ind_plan((Final-Start)/1000000000.0);
	.print("Elapsed time individual planning round ",Round,": ",(Final-Start)/1000000000.0);
	...
	.stopMAS;
	.
+!count : count(C) & replan(R) & C + R == 20  & round(Round) & time(Start)
<-
	-count(C);
	+count(0);
	if (R == 0) {
