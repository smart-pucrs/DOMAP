...
goal(deliver_box(box1,area1)).
goal(deliver_box(box2,area27)).
goal(deliver_box(box3,area20)).
goal(deliver_box(box4,area15)).
goal(deliver_box(box5,area9)).
goal(deliver_sample(cdm2,area18)).
goal(deliver_sample(cdm1,area14)).
goal(deliver_sample(cdm3,area2)).
goal(deliver_sample(cdm3,area26)).
goal(deliver_sample(cdm1,area12)).
...
+!replan(Goal,Role) : count(C) & replan(R) & C + R == 17 & round(Round) & time(Start)
<-
	-count(C);
	+count(0);
	if (R == 0) {
...
+!count : count(C) & C == 17 & time(Start) & replan(0) & round(Round)
<-
	jia.time(Start,Final);
	+time_ind_plan((Final-Start)/1000000000.0);
	.print("Elapsed time individual planning round ",Round,": ",(Final-Start)/1000000000.0);
	...
	.stopMAS;
	.
+!count : count(C) & replan(R) & C + R == 17  & round(Round) & time(Start)
<-
	-count(C);
	+count(0);
	if (R == 0) {
