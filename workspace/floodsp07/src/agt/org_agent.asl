...
goal(deliver_box(box1,area24)).
goal(deliver_box(box2,area28)).
goal(deliver_box(box3,area34)).
goal(deliver_box(box4,area6)).
goal(deliver_box(box5,area20)).
goal(deliver_box(box6,area44)).
goal(deliver_box(box7,area30)).
goal(deliver_box(box8,area40)).
goal(deliver_sample(cdm2,area6)).
goal(deliver_sample(cdm1,area29)).
goal(deliver_sample(cdm3,area17)).
goal(deliver_sample(cdm3,area44)).
goal(deliver_sample(cdm4,area8)).
goal(deliver_sample(cdm5,area34)).
goal(deliver_sample(cdm5,area2)).
goal(deliver_sample(cdm1,area41)).
...
+!replan(Goal,Role) : count(C) & replan(R) & C + R == 26 & round(Round) & time(Start)
<-
	-count(C);
	+count(0);
	if (R == 0) {
...
+!count : count(C) & C == 26 & time(Start) & replan(0) & round(Round)
<-
	jia.time(Start,Final);
	+time_ind_plan((Final-Start)/1000000000.0);
	.print("Elapsed time individual planning round ",Round,": ",(Final-Start)/1000000000.0);
	...
	.stopMAS;
	.
+!count : count(C) & replan(R) & C + R == 26  & round(Round) & time(Start)
<-
	-count(C);
	+count(0);
	if (R == 0) {
